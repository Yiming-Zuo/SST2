...
def create_system_simulation(pdb_file,
                             forcefield,
...
class Rest2Reporter(object):
    def __init__(self, file, reportInterval, rest2):
        ...
        self._out.write("ps,Solute scaled(kJ/mol),Solute not scaled(kJ/mol),Solvent(kJ/mol),Solute-Solvent(kJ/mol)\n")
    ...
    def report(self, simulation, state):
        ...
        self._out.write(f'{time},{energies[0]._value},{energies[1]._value},{energies[2]._value},{energies[3]._value}\n') 
...
class REST2():
    ...
    def __init__(self,
        ...
        self.solvent_index = list(set(range(self.system.getNumParticles())).difference(set(self.solute_index)))
        self.system_forces = {type(force).__name__ : force for force in self.system.getForces()}
        self.scale = 1.0
    ...
    def create_solute_solvent_simulation(self,
                                         forcefield,
        ...
        # Save pdb coordinates to read them with pdb_manip
        ...
        # Read 
        solute_solvent_coor = pdb_manip.Coor()
        solute_solvent_coor.parse_pdb_lines(new_stdout.getvalue().split('\n'))

        # Separate coordinates in two pdb files:
        solute_coor = copy.deepcopy(solute_solvent_coor)
        self.solvent_index = [i for i in self.solvent_index if i < top_num_atom]
        solute_coor.del_atom_index(self.solvent_index)
        solute_coor.write_pdb(solute_out_pdb, check_file_out=False)

        solvent_coor = copy.deepcopy(solute_solvent_coor)
        solvent_coor.del_atom_index(self.solute_index)
        solvent_coor.write_pdb(solvent_out_pdb, check_file_out=False)
    ...
    def setup_simulation(self, integrator,
                         temperature = 300 * unit.kelvin,
                         pressure = 1.0 * unit.atmospheres,
                         barostatInterval=25,
                         platform_name='CUDA',
        ...
        self.system.addForce(MonteCarloBarostat(pressure, temperature, barostatInterval))
    ...
    def compute_solute_solvent_system_energy(self):
        ...
        self.simulation_solute.context.setPeriodicBoxVectors(
            *box_vector)
        self.simulation_solute.context.setPositions(
            tot_positions[self.solute_index])

        forces_solute = get_forces(
            self.system_solute,
            self.simulation_solute)

        self.simulation_solvent.context.setPeriodicBoxVectors(
            *box_vector)
        self.simulation_solvent.context.setPositions(
            tot_positions[self.solvent_index])

        forces_solvent = get_forces(
            self.system_solvent,
            self.simulation_solvent)
    ...
    def separate_torsion_pot(self):
        ...
        for i in range(original_torsion_force.getNumTorsions()):
            p1, p2, p3, p4, periodicity, phase, k = original_torsion_force.getTorsionParameters(i)
            ...
            if solute_in and not_improper:
                solute_scaled_torsion_force.addTorsion(p1, p2, p3, p4, [periodicity, phase, k])
            elif solute_in:
                solute_not_scaled_torsion_force.addTorsion(p1, p2, p3, p4, [periodicity, phase, k])                
            elif solvent_in:
                solvent_torsion_force.addTorsion(p1, p2, p3, p4, [periodicity, phase, k])
            else:
                print('Wrong Torsion !')
                exit()
    ...
    def find_solute_nb_index(self):
        ...
        for exception_index in range(nonbonded_force.getNumExceptions()):
            ...
            if (iatom in self.solute_index and jatom in self.solute_index) :
                ...
                self.init_nb_exept_value.append([iatom, jatom, chargeprod, sigma, epsilon])
    ...
    def find_nb_solute_system(self):
        ...
        for exception_index in range(nonbonded_force.getNumExceptions()):
            [iatom, jatom, chargeprod, sigma, epsilon] = nonbonded_force.getExceptionParameters(exception_index)
            self.init_nb_exept_solute_value.append([iatom, jatom, chargeprod, sigma, epsilon])
    ...
    def update_nonbonded(self, scale):
        ...
        nonbonded_force.updateParametersInContext(
    ...
    def update_nonbonded_solute(self, scale):
        ...
        # Need to fix simulation 
        nonbonded_force.updateParametersInContext(
            self.simulation_solute.context)
    ...
    def compute_all_energies(self):
        ...
        solvent_term = ['HarmonicBondForce', 'HarmonicAngleForce',
        ...
        system_term = ['HarmonicBondForce', 'HarmonicAngleForce',
    ...
    def add_custom_LJ_forces(self):
        ...
        # Copy solute-solute exclusions
        for exception_index in range(nonbonded_force.getNumExceptions()):
            [iatom, jatom, chargeprod, sigma, epsilon] = nonbonded_force.getExceptionParameters(
                exception_index)
    ...
    def add_custom_Coulomb_forces(self):
        ...
        energy_expression += "- ONE_4PI_EPS0*chargeProd_product * erf(alpha_ewald * r) / r;"
        energy_expression += "ONE_4PI_EPS0 = {:f};".format(ONE_4PI_EPS0)  # already in OpenMM units
        energy_expression += "alpha_ewald = {:f};".format(alpha_ewald.value_in_unit_system(unit.md_unit_system))
        custom_bond_force = openmm.CustomBondForce(energy_expression)
    ...
    def add_custom_solute_nb_forces(self, solvent, solute):
        ...
        for exception_index in range(nonbonded_force.getNumExceptions()):
            ...
            if solvent == solute:
                ...
                if (iatom in solute and jatom in solute) :#or (iatom in solvent and jatom in solvent):
        ...
        self.add_negative_force(
            ...
            name="NegativeCustomNonbondedForce")
    ...
    def update_custom_nonbonded(self, scale):
        ...
        # Need to fix simulation 
        custom_nonbonded_force.updateParametersInContext(
            self.simulation.context)
...
if __name__ == "__main__":
    ...
    simulation = setup_simulation(system, pdb.positions, pdb.topology, integrator, 'CUDA')
    ...
    system_pep = forcefield.createSystem(pdb_pep.topology, 
                                        nonbondedMethod=PME, 
                                        nonbondedCutoff=1*unit.nanometers, 
                                        constraints=HBonds)

    simulation_pep = setup_simulation(system_pep,
        pdb_pep.positions, pdb_pep.topology, integrator_pep, 'CUDA')
    ...
    system_no_pep = forcefield.createSystem(pdb_no_pep.topology, 
                                        nonbondedMethod=PME, 
                                        nonbondedCutoff=1*unit.nanometers, 
                                        constraints=HBonds)

    simulation_no_pep = setup_simulation(system_no_pep,
        pdb_no_pep.positions, pdb_no_pep.topology, integrator_no_pep, 'CUDA')
    ...
    solvent_indices = [int(i.index) for i in pdb.topology.atoms() if not (i.residue.chain.id in ['B'])]
    solute_indices = [int(i.index) for i in pdb.topology.atoms() if i.residue.chain.id in ['B']]
    ...
    E_solute_scaled, E_solute_not_scaled, E_solvent, solvent_solute_nb = test.compute_all_energies()
    ...
    print(f"HarmonicBondForce    {forces_rest2[0]['energy']/forces_sys[0]['energy']:.5e}")
    print(f"HarmonicAngleForce   {forces_rest2[1]['energy']/forces_sys[1]['energy']:.5e}")
    print('Compare scaled energy:')
    torsion_force = forces_rest2[4]['energy'] + forces_rest2[5]['energy'] + forces_rest2[6]['energy']
    print(f"PeriodicTorsionForce {torsion_force/forces_sys[3]['energy']:.5e}")
    print(f"NonbondedForce       {forces_rest2[2]['energy']/forces_sys[2]['energy']:.5e}")
    print(f"Total                {forces_rest2[9]['energy']/forces_sys[6]['energy']:.5e}")
    ...
    non_bonded =  solvent_solute_nb + forces_pep[2]['energy'] + forces_no_pep[2]['energy']
    ...
    solute_not_scaled_force = forces_rest2[5]['energy'] + forces_pep[0]['energy'] + + forces_pep[1]['energy']
    ...
    print(f"HarmonicAngleForce   {forces_rest2[1]['energy']/forces_sys[1]['energy']:.5e}")
    print('Compare scaled energy:')
    torsion_force = forces_rest2[4]['energy'] * scale + forces_rest2[5]['energy'] + forces_rest2[6]['energy']
    print(f"PeriodicTorsionForce {torsion_force/forces_sys[3]['energy']:.5e}")
    print(f"NonbondedForce       {forces_rest2[2]['energy']/forces_sys[2]['energy']:.5e}")
    print(f"Total                {forces_rest2[9]['energy']/forces_sys[6]['energy']:.5e}")
